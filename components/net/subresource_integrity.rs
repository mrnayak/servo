...
fn parsed_metadata(integrity_metadata: &str) -> Vec<SriEntry> {
    // Step 1
    let mut result = vec![];
...
fn get_prioritized_hash_function(hash_func_left: &str, hash_func_right: &str) -> String {
...
fn get_strongest_metadata(integrity_metadata_list: Vec<SriEntry>) -> Vec<SriEntry> {
    let mut result: Vec<SriEntry> = vec![integrity_metadata_list[0].clone()];
    let ref mut current_algorithm = result[0].alg.clone();

    for i in 1..integrity_metadata_list.len() {
        let intgerity_metadata = integrity_metadata_list[i].clone();

        let prioritized_metadata = get_prioritized_hash_function(&intgerity_metadata.alg,
                                                                 &*current_algorithm);
        if prioritized_metadata == "" {
            result.push(intgerity_metadata.clone());
        } else if prioritized_metadata != *current_algorithm {
